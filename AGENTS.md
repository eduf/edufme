...
When pushing from a Codex terminal session, use `gh auth login` first (browser-based flow). GitHub does not accept plain passwords — use a Personal Access Token or the `gh` CLI. If you get a "push declined due to email privacy restrictions" error, amend the offending commit:
